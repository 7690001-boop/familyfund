...
export function render(container, investments, { familyId, sym = '₪', canEdit = false, canNote = false, canRename = false, canSell = false, onEdit, onSell } = {}) {
    ...
    if (!investments || investments.length === 0) {
        container.innerHTML = `<div class="heatmap-empty">🌱<br>עוד אין השקעות</div>`;
        return;
    }
    ...
    const tilesHtml = positions.map((pos, i) => {
        ...
        const flexVal = Math.max(10, Math.round(pct * 100));
        const minW = flexVal < 20 ? '80px' : flexVal < 40 ? '120px' : '160px';
        ...
        return `<div class="heatmap-tile"
            style="flex: ${flexVal}; min-width: min(${minW}, 100%); background: ${color.bg}; border-color: ${color.border};"
            data-pos-idx="${i}"
            role="button"
            tabindex="0">
    ...
    container.innerHTML = `<div class="heatmap-grid">${tilesHtml}</div>`;
    ...
    openModal(`
        <div class="heatmap-detail">
            ...
            <div class="heatmap-detail-grid">
                ...
                <div class="heatmap-detail-card ${plClass}">
                ...
                <div class="heatmap-detail-card">
                    <div class="heatmap-detail-label">מחיר קנייה ממוצע</div>
